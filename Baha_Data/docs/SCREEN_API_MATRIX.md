...
| ST-006 | Home Dashboard | Show recommended next steps and the fastest support entry points | `GET /mobile/me`, `GET /mobile/student/weekly-summary/latest`, `GET /mobile/student/checkins`, `GET /mobile/student/checkins/{response_set_id}` | Active student | Theme mode, dashboard shell | Ready | `GET /mobile/me` now returns student metadata including persisted wellbeing baseline answers; the home screen now prioritizes `Recommended next`, then `SOS Help`, `BAHA Buddy`, and `Daily Check-in`, while deeper weekly scores moved to a separate `Your Week` screen |
...
| ST-008 | Check-In Detail | Render dynamic check-in form with conditional follow-ups | `GET /mobile/student/checkin-templates/{template_id}` | Active student | In-progress answer draft | Ready | Backend supplies ordered questions and metadata; client applies age-aware wording and evaluates `show_when` logic while keeping the six stored factor dimensions stable |
| ST-009 | Check-In Submit | Persist adaptive answers and normalized scores | `POST /mobile/student/checkins` | Active student | Submission retry state | Ready | Student app submits selected options plus normalized scoring metadata and now shows a short post-submit takeaway based on today's factors versus recent averages |
| ST-010 | Your Week | Expanded student analytics with multiple time windows and evidence-led trend notes | `GET /mobile/student/weekly-summary/latest`, `GET /mobile/student/checkins`, `GET /mobile/student/checkins/{response_set_id}` | Active student | Selected timeframe, selected factor | Ready | Shows a separate weekly view with `7D` / `14D` / `30D` / all-history windows, factor-specific charts, and short conclusions backed by visible evidence instead of score-heavy summary cards |
| ST-011 | Learn Feed | Discovery cards and modules | `GET /mobile/content/feed`, `GET /mobile/student/modules` | Active student | Sort/filter state | Ready | Supports theme-focused lanes via `theme`; the student experience now separates Learn from Activities instead of mixing content and tools into one grid |
...
| ST-014 | Buddy Chat | Persist question/response thread with progressive assistant rendering | `GET /mobile/chat/sessions/{session_id}/messages`, `POST /mobile/chat/sessions/{session_id}/messages`, `POST /mobile/chat/sessions/{session_id}/messages/stream` | Active student | Draft message, scroll state, in-flight stream state | Ready | Buddy now uses OpenAI for all normal replies, retrieval only when grounding is useful, the stream route emits progressive `ack` / `delta` / `complete` events, the backend injects short remembered session context for better continuity inside one conversation, and student tone now adapts by age cohort |
...
| ST-016 | Profile and Settings | Theme, privacy reminders, session actions, wellbeing profile editing, student ID, guardian pairing, parent-summary privacy control, and unpairing | `GET /mobile/me`, `GET /auth/onboarding-state`, `POST /auth/bootstrap`, `GET /mobile/student/linking-state`, `POST /mobile/student/linking-code`, `POST /mobile/student/parent-summary-sharing`, `DELETE /mobile/student/guardians/{guardian_id}` | Active student | Theme and app prefs | Ready | Settings now supports selectable color palettes, student baseline edits through bootstrap metadata refresh, visible student ID, on-demand 6-digit pairing codes, the student-controlled parent-summary sharing toggle, and confirmed unlink actions for linked guardians |
| ST-017 | Avatar Selection | Visual identity only | None initially | Active student | Selected avatar | Ready | Frontend-local for now |
| ST-018 | Activities Hub | Entry to Comet Sequence, Calm Breathing, and Focus Catch | Local-only tools | Active student | Animation state, local completion state | Ready | The current demo keeps the activities slice intentionally lightweight: two cognitive mini-games plus one paced breathing tool |
| ST-019 | Journal | Private reflection, guided prompts, gratitude notes, favorites, and history | Local-first persistence via `shared_preferences` | Active student | Draft entry, filters, reminder toggle, favorites | Ready | The unified app now includes a local journal with themed route persistence, separate free-write / guided / gratitude flows, search, streaks, and saved entries |
...
| PA-003 | Parent Home | Show quick parent support, linked children, and summary entry points | `GET /mobile/parent/students`, `GET /mobile/parent/students/{student_profile_id}/weekly-summary/latest`, `GET /mobile/support-contacts` | Active guardian | Selected child context | Ready | Core parent dashboard path; now leads with `Parent Buddy` and `SOS Help`, uses `linked children` wording, and keeps summary access in a separate child-focused preview flow instead of front-loading analytics |
| PA-004 | Weekly Summary Detail | Show consent-gated conversation-safe summary | `GET /mobile/parent/students/{student_profile_id}/weekly-summary/latest` | Active guardian with consent | Expand/collapse UI | Ready | No raw student data; now optimized into shorter weekly signals, compact counts, a conversation starter, and a next support step |
...
| PA-008 | Parent Settings | Linked student management and privacy reminders | `GET /mobile/parent/students`, `DELETE /mobile/parent/students/{student_profile_id}/link`, `GET /auth/onboarding-state`, `GET /mobile/support-contacts` | Active guardian | Local preference state | Partial | Rich privacy-tier editor still not fully modeled; unlinking is now live from the parent side with confirmation |
...
- Story World is currently removed from the active demo path and should be treated as dormant, not part of the presentable release slice
