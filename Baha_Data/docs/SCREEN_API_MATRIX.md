...
| ST-010 | Your Week | Expanded weekly progress view with clearer quantitative scoring | `GET /mobile/student/weekly-summary/latest`, `GET /mobile/student/checkins`, `GET /mobile/student/checkins/{response_set_id}` | Active student | Chart tab state | Ready | Shows a separate weekly view with line chart, area-by-area scores, and compact number-led takeaways instead of placing those analytics on the home dashboard |
...
| ST-019 | Journal | Private reflection, guided prompts, favorites, and history | Local-first persistence via `shared_preferences` | Active student | Draft entry, filters, reminder toggle, favorites | Ready | The unified app now includes a local journal with free-write, guided, and gratitude modes plus search, streaks, and saved entries |
...
| PA-003 | Parent Home | Show linked students and current summary | `GET /mobile/parent/students`, `GET /mobile/parent/students/{student_profile_id}/weekly-summary/latest` | Active guardian | Selected child context | Ready | Core parent dashboard path; now keeps the summary layer shorter with mini-stats and support actions instead of long narrative blocks, while still avoiding raw student entries |
