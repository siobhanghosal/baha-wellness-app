...
| SH-003 | Account Bootstrap | All | Create BAHA-side profile | `POST /auth/bootstrap` | Authenticated identity | Draft form state | Ready | Core entry to all apps |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| ST-001 | Welcome | Trust framing and entry | `GET /auth/onboarding-state` | Student identity | Intro animation | Partial | Content copy still mostly client-owned |
| ST-002 | Age and Profile Setup | Capture age cohort, consent band, gender | `POST /auth/bootstrap` | Student identity | Draft onboarding form | Ready | Must follow FR-OB-001 |
...
| ST-011 | Learn Feed | Discovery cards and modules | `GET /mobile/content/feed`, `GET /mobile/student/modules` | Active student | Sort/filter state | Ready | Use content feed plus modules together |
| ST-012 | Module Detail | Show module body and progress | `GET /mobile/content/{content_item_id}`, `POST /mobile/student/modules/{module_id}/progress` | Active student | Step progress, optimistic UI | Ready | `GET /mobile/student/modules` now exposes `content_item_id` directly for stable client routing |
...
| PA-002 | Student Linking | Link parent to student | `POST /auth/guardian/link-student`, `GET /auth/onboarding-state` | Active guardian | Draft student code | Ready | Important blocked state if not linked |
...
| PA-006 | Platform Participation Consent | Activate minor student access when required | `POST /auth/guardian/consent/platform-participation` | Active guardian with consent authority | Confirmation UI | Ready | Important onboarding unblocker |
