...
| ST-014 | Buddy Chat | Persist question/response thread with progressive assistant rendering | `GET /mobile/chat/sessions/{session_id}/messages`, `POST /mobile/chat/sessions/{session_id}/messages`, `POST /mobile/chat/sessions/{session_id}/messages/stream` | Active student | Draft message, scroll state, in-flight stream state | Ready | Buddy now uses OpenAI for all normal replies, retrieval only when grounding is useful, the stream route emits progressive `ack` / `delta` / `complete` events, and the backend injects short remembered session context for better continuity inside one conversation |
...
| PA-003 | Parent Home | Show linked students and current summary | `GET /mobile/parent/students`, `GET /mobile/parent/students/{student_profile_id}/weekly-summary/latest` | Active guardian | Selected child context | Ready | Core parent dashboard path; now includes a privacy-safe action layer built from the weekly summary rather than raw student entries |
