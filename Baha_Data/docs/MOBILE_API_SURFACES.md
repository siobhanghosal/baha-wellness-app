...
For the live unified-app flow, the guardian must also provide the six-digit verification code generated by the student. Adult students can use the same pairing flow; under-18 students still require guardian platform approval before the student account becomes active.
...
Returns the student-facing pairing and privacy state:

- `student_code`
- current six-digit guardian-link verification code if one exists
- code generation and expiry timestamps
- linked guardian count
- parent-summary sharing toggle state

`POST /mobile/student/linking-code`

Generates or refreshes the student’s six-digit guardian-link verification code. The current local behavior is a 30-minute expiry window.

`POST /mobile/student/parent-summary-sharing`

Updates whether the student allows guardians to view summary charts at all. Guardian summary access still also requires a linked guardian plus guardian-side parent-summary consent.

`DELETE /mobile/student/guardians/{guardian_id}`

Removes one active parent/guardian link from the student side. The intended UI behavior is a confirmation step before the request is sent.
...
Removes the active link between the current guardian and one student. The intended UI behavior is a confirmation step before the request is sent.
