...
- Screen ID: `S-06`
- Screen Name: `Parent Consent Pending`
- Description: Blocks data collection until guardian approval arrives.
...
- Header: Top app bar with age-appropriate title, back affordance when not top-level, and optional help action. Screen title is 'Parent Consent Pending'.
...
- Cards: Use cards to present the main units of information on 'Parent Consent Pending'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Parent Consent Pending' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Parent Consent Pending' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/Student_Parent_Consent_Pending.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Student_Parent_Consent_Pending.mmd)
