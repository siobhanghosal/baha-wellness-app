...
- Description: Shows signal origin, event timeline, linked context, and notes.
...
- Header: Dense operational header with role context, queue counters, filters, and export or action shortcuts. Screen title is 'Case Detail'.
...
- Cards: Use cards to present the main units of information on 'Case Detail'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Case Detail' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Case Detail' is missing or delayed.
...
- Diagram File: [Mermaid/BAHA_Case_Detail.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/BAHA_Case_Detail.mmd)
