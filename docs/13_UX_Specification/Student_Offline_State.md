...
- Screen ID: `S-39`
- Screen Name: `Offline State`
- Description: Supports cached check-ins, calming activities, and text/audio learning.
...
- Connectivity is missing or degraded enough to block live data retrieval.
- Exit Conditions:
...
- Automatic fallback after failed live fetch
- Navigation Destinations:
- Previous screen via back navigation
- Retry same screen
- Cached parent screen
- Global offline help
- Incoming Screens:
- App bootstrap router
- Back stack return path
...
- Header: Top app bar with age-appropriate title, back affordance when not top-level, and optional help action. Screen title is 'Offline State'.
...
- Cards: Use cards to present the main units of information on 'Offline State'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Offline State' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Offline State' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/Student_Offline_State.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Student_Offline_State.mmd)
