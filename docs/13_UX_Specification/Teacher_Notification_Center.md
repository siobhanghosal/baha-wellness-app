...
- Description: Operational alerts for training, referrals, and policy updates.
...
- Review Policy
- Button Destinations:
- Save Changes: Notification Center
...
- Header: Operational app bar with school context, cohort selector, and referral/notification shortcuts. Screen title is 'Notification Center'.
...
- Cards: Use cards to present the main units of information on 'Notification Center'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Notification Center' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Notification Center' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/Teacher_Notification_Center.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Teacher_Notification_Center.mmd)
