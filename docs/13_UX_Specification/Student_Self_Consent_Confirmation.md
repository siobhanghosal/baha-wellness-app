...
- Screen ID: `S-07`
- Screen Name: `Self-Consent Confirmation`
- Description: Confirms 18-19 user consent and data rights.
...
- Header: Top app bar with age-appropriate title, back affordance when not top-level, and optional help action. Screen title is 'Self-Consent Confirmation'.
...
- Cards: Use cards to present the main units of information on 'Self-Consent Confirmation'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Self-Consent Confirmation' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Self-Consent Confirmation' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/Student_Self_Consent_Confirmation.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Student_Self_Consent_Confirmation.mmd)
