...
- Screen ID: `B-02`
- Screen Name: `Queue Filters`
- Description: Filters by source, severity, school, age band, and status.
...
- Header: Dense operational header with role context, queue counters, filters, and export or action shortcuts. Screen title is 'Queue Filters'.
...
- Cards: Use cards to present the main units of information on 'Queue Filters'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Queue Filters' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Queue Filters' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/BAHA_Queue_Filters.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/BAHA_Queue_Filters.mmd)
