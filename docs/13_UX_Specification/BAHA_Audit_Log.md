...
- Description: Shows access, consent, escalation, and content actions.
...
- Header: Dense operational header with role context, queue counters, filters, and export or action shortcuts. Screen title is 'Audit Log'.
...
- Cards: Use cards to present the main units of information on 'Audit Log'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Audit Log' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Audit Log' is missing or delayed.
...
- Diagram File: [Mermaid/BAHA_Audit_Log.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/BAHA_Audit_Log.mmd)
