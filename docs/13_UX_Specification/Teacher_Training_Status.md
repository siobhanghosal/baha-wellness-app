...
- Screen ID: `T-02`
- Screen Name: `Training Status`
- Description: Gates sensitive features until required learning is complete.
...
- Header: Operational app bar with school context, cohort selector, and referral/notification shortcuts. Screen title is 'Training Status'.
...
- Cards: Use cards to present the main units of information on 'Training Status'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Training Status' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Training Status' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/Teacher_Training_Status.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Teacher_Training_Status.mmd)
