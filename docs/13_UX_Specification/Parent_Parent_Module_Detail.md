...
- Screen ID: `P-10`
- Screen Name: `Parent Module Detail`
- Description: Displays objectives, duration, and completion state.
...
- Header: Clear summary-first app bar with title, linked-student switcher when multiple children are linked, and notification entry point. Screen title is 'Parent Module Detail'.
...
- Cards: Use cards to present the main units of information on 'Parent Module Detail'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Parent Module Detail' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Parent Module Detail' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/Parent_Parent_Module_Detail.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Parent_Parent_Module_Detail.mmd)
