...
- Description: Routes to BAHA support, hotline, and emergency guidance.
...
- Header: Top app bar with age-appropriate title, back affordance when not top-level, and optional help action. Screen title is 'Help Center'.
...
- Cards: Use cards to present the main units of information on 'Help Center'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Help Center' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Help Center' is missing or delayed.
...
- Diagram File: [Mermaid/Student_Help_Center.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Student_Help_Center.mmd)
