# S-22 - Friendship Choices

## Core Identity

- Screen ID: `S-22`
- Screen Name: `Friendship Choices`
- Description: Narrative peer-pressure and social reasoning game.
...
- Take a Break
- Button Destinations:
- Start: Friendship Choices
- Resume: Friendship Choices
- Take a Break: Home or advisory break prompt
- Gesture Navigation:
...
- Header: Top app bar with age-appropriate title, back affordance when not top-level, and optional help action. Screen title is 'Friendship Choices'.
...
- Cards: Use cards to present the main units of information on 'Friendship Choices'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Friendship Choices' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- game_started
- game_completed
- time_cap_prompt_shown
- screen_view:S-22
- Logging Events:
- game_session_start
- game_signal_aggregate
- time_governance_result
- ux_spec_reference:S-22
...
- Breathing or gameplay motion must support rhythm and clarity rather than reward-seeking overstimulation.
- Microinteractions:
...
- Edge Cases:
- Upstream payload for 'Friendship Choices' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/Student_Friendship_Choices.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Student_Friendship_Choices.mmd)
