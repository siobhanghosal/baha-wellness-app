# S-14 - Trend Dashboard Empty

## Core Identity

- Screen ID: `S-14`
- Screen Name: `Trend Dashboard Empty`
- Description: Explains why more data is needed before insights appear.
...
- Header: Top app bar with age-appropriate title, back affordance when not top-level, and optional help action. Screen title is 'Trend Dashboard Empty'.
...
- Cards: Use cards to present the main units of information on 'Trend Dashboard Empty'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Trend Dashboard Empty' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Trend Dashboard Empty' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/Student_Trend_Dashboard_Empty.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Student_Trend_Dashboard_Empty.mmd)
