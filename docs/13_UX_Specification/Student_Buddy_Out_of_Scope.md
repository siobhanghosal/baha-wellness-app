...
- Screen ID: `S-32`
- Screen Name: `Buddy Out-of-Scope`
- Description: Safe refusal and redirection state.
...
- Header: Top app bar with age-appropriate title, back affordance when not top-level, and optional help action. Screen title is 'Buddy Out-of-Scope'.
...
- Cards: Use cards to present the main units of information on 'Buddy Out-of-Scope'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Buddy Out-of-Scope' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Buddy Out-of-Scope' is missing or delayed.
...
- Diagram File: [Mermaid/Student_Buddy_Out_of_Scope.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Student_Buddy_Out_of_Scope.mmd)
