...
- Screen ID: `P-06`
- Screen Name: `Weekly Summary Home`
- Description: Shows the current week's aggregate summary.
...
- Header: Clear summary-first app bar with title, linked-student switcher when multiple children are linked, and notification entry point. Screen title is 'Weekly Summary Home'.
...
- Cards: Use cards to present the main units of information on 'Weekly Summary Home'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Weekly Summary Home' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Weekly Summary Home' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/Parent_Weekly_Summary_Home.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Parent_Weekly_Summary_Home.mmd)
