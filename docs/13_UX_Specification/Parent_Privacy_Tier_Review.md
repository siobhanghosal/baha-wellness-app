# P-05 - Privacy Tier Review

## Core Identity

- Screen ID: `P-05`
- Screen Name: `Privacy Tier Review`
- Description: Displays and negotiates allowed summary tiers.
...
- Header: Clear summary-first app bar with title, linked-student switcher when multiple children are linked, and notification entry point. Screen title is 'Privacy Tier Review'.
...
- Cards: Use cards to present the main units of information on 'Privacy Tier Review'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Privacy Tier Review' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Privacy Tier Review' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/Parent_Privacy_Tier_Review.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Parent_Privacy_Tier_Review.mmd)
