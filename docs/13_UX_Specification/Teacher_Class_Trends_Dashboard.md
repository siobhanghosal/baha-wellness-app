# T-03 - Class Trends Dashboard

## Core Identity

- Screen ID: `T-03`
- Screen Name: `Class Trends Dashboard`
- Description: Weekly anonymized cohort trends by theme.
...
- Refresh: Class Trends Dashboard
...
- Header: Operational app bar with school context, cohort selector, and referral/notification shortcuts. Screen title is 'Class Trends Dashboard'.
...
- Cards: Use cards to present the main units of information on 'Class Trends Dashboard'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Class Trends Dashboard' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Class Trends Dashboard' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/Teacher_Class_Trends_Dashboard.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Teacher_Class_Trends_Dashboard.mmd)
