# S-12 - Check-In Questionnaire

## Core Identity

- Screen ID: `S-12`
- Screen Name: `Check-In Questionnaire`
- Description: Captures mood, sleep, energy, stress, screen time, physical activity, lifestyle, and academic stress.
...
- Header: Top app bar with age-appropriate title, back affordance when not top-level, and optional help action. Screen title is 'Check-In Questionnaire'.
...
- Cards: Use cards to present the main units of information on 'Check-In Questionnaire'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Check-In Questionnaire' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Check-In Questionnaire' is missing or delayed.
...
- Diagram File: [Mermaid/Student_Check_In_Questionnaire.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Student_Check_In_Questionnaire.mmd)
