# P-09 - Parent Learning Home

## Core Identity

- Screen ID: `P-09`
- Screen Name: `Parent Learning Home`
- Description: Shows modules on adolescent wellbeing and app usage.
...
- Header: Clear summary-first app bar with title, linked-student switcher when multiple children are linked, and notification entry point. Screen title is 'Parent Learning Home'.
...
- Cards: Use cards to present the main units of information on 'Parent Learning Home'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Parent Learning Home' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Parent Learning Home' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/Parent_Parent_Learning_Home.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Parent_Parent_Learning_Home.mmd)
