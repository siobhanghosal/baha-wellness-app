# S-02 - Age-Band Selection

## Core Identity

- Screen ID: `S-02`
- Screen Name: `Age-Band Selection`
- Description: Captures presentation cohort without revealing full DOB in the primary UI.
...
- Header: Top app bar with age-appropriate title, back affordance when not top-level, and optional help action. Screen title is 'Age-Band Selection'.
...
- Cards: Use cards to present the main units of information on 'Age-Band Selection'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Age-Band Selection' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Age-Band Selection' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/Student_Age_Band_Selection.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Student_Age_Band_Selection.mmd)
