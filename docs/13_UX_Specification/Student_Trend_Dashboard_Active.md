...
- Screen ID: `S-15`
- Screen Name: `Trend Dashboard Active`
- Description: Displays supportive, non-diagnostic 4-week trend summaries.
...
- Refresh: Trend Dashboard Active
...
- Header: Top app bar with age-appropriate title, back affordance when not top-level, and optional help action. Screen title is 'Trend Dashboard Active'.
...
- Cards: Use cards to present the main units of information on 'Trend Dashboard Active'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Trend Dashboard Active' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Trend Dashboard Active' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/Student_Trend_Dashboard_Active.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Student_Trend_Dashboard_Active.mmd)
