...
- Screen ID: `S-03`
- Screen Name: `Legal Consent Routing`
- Description: Determines minor flow versus self-consent flow.
...
- Header: Top app bar with age-appropriate title, back affordance when not top-level, and optional help action. Screen title is 'Legal Consent Routing'.
...
- Cards: Use cards to present the main units of information on 'Legal Consent Routing'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Legal Consent Routing' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Legal Consent Routing' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/Student_Legal_Consent_Routing.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Student_Legal_Consent_Routing.mmd)
