...
- Screen ID: `S-21`
- Screen Name: `Emotion Explorer`
- Description: Scenario-based emotional literacy gameplay.
...
- Take a Break
- Button Destinations:
- Start: Emotion Explorer
- Resume: Emotion Explorer
...
- Header: Top app bar with age-appropriate title, back affordance when not top-level, and optional help action. Screen title is 'Emotion Explorer'.
...
- Cards: Use cards to present the main units of information on 'Emotion Explorer'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Emotion Explorer' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Emotion Explorer' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/Student_Emotion_Explorer.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Student_Emotion_Explorer.mmd)
