...
- Screen ID: `P-14`
- Screen Name: `Offline Summary Placeholder`
- Description: Indicates summaries require sync while preserving cached policy guidance.
...
- Refresh: Offline Summary Placeholder
...
- Header: Clear summary-first app bar with title, linked-student switcher when multiple children are linked, and notification entry point. Screen title is 'Offline Summary Placeholder'.
...
- Cards: Use cards to present the main units of information on 'Offline Summary Placeholder'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Offline Summary Placeholder' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Offline Summary Placeholder' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/Parent_Offline_Summary_Placeholder.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Parent_Offline_Summary_Placeholder.mmd)
