# B-09 - Content Review Queue

## Core Identity

- Screen ID: `B-09`
- Screen Name: `Content Review Queue`
- Description: Clinical review and publish gating.
...
- Header: Dense operational header with role context, queue counters, filters, and export or action shortcuts. Screen title is 'Content Review Queue'.
...
- Cards: Use cards to present the main units of information on 'Content Review Queue'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Content Review Queue' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Content Review Queue' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/BAHA_Content_Review_Queue.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/BAHA_Content_Review_Queue.mmd)
