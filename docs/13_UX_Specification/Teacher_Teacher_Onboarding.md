...
- Screen ID: `T-01`
- Screen Name: `Teacher Onboarding`
- Description: Explains safeguards, role limits, and dashboard meaning.
...
- Header: Operational app bar with school context, cohort selector, and referral/notification shortcuts. Screen title is 'Teacher Onboarding'.
...
- Cards: Use cards to present the main units of information on 'Teacher Onboarding'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Teacher Onboarding' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Teacher Onboarding' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/Teacher_Teacher_Onboarding.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Teacher_Teacher_Onboarding.mmd)
