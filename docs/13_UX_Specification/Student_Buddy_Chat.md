...
- Screen ID: `S-30`
- Screen Name: `Buddy Chat`
- Description: Main conversational interface with citations and escalation affordances.
...
- Header: Top app bar with age-appropriate title, back affordance when not top-level, and optional help action. Screen title is 'Buddy Chat'.
...
- Cards: Use cards to present the main units of information on 'Buddy Chat'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Buddy Chat' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Buddy Chat' is missing or delayed.
...
- Diagram File: [Mermaid/Student_Buddy_Chat.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Student_Buddy_Chat.mmd)
