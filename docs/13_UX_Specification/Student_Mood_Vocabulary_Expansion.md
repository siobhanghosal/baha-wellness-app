...
- Screen ID: `S-17`
- Screen Name: `Mood Vocabulary Expansion`
- Description: Introduces richer emotion words over time.
...
- Header: Top app bar with age-appropriate title, back affordance when not top-level, and optional help action. Screen title is 'Mood Vocabulary Expansion'.
...
- Cards: Use cards to present the main units of information on 'Mood Vocabulary Expansion'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Mood Vocabulary Expansion' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Mood Vocabulary Expansion' is missing or delayed.
...
- Diagram File: [Mermaid/Student_Mood_Vocabulary_Expansion.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Student_Mood_Vocabulary_Expansion.mmd)
