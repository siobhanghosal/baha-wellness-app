...
- Screen ID: `B-14`
- Screen Name: `Analytics Export`
- Description: Creates pilot reporting extracts.
...
- Header: Dense operational header with role context, queue counters, filters, and export or action shortcuts. Screen title is 'Analytics Export'.
...
- Cards: Use cards to present the main units of information on 'Analytics Export'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Analytics Export' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Analytics Export' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/BAHA_Analytics_Export.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/BAHA_Analytics_Export.mmd)
