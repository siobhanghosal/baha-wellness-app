# S-05 - Privacy Promise

## Core Identity

- Screen ID: `S-05`
- Screen Name: `Privacy Promise`
- Description: Explains what stays private, what may be summarized, and override rules.
...
- Header: Top app bar with age-appropriate title, back affordance when not top-level, and optional help action. Screen title is 'Privacy Promise'.
...
- Cards: Use cards to present the main units of information on 'Privacy Promise'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Privacy Promise' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Privacy Promise' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/Student_Privacy_Promise.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Student_Privacy_Promise.mmd)
