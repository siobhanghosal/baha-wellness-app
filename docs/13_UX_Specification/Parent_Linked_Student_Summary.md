...
- Screen ID: `P-03`
- Screen Name: `Linked Student Summary`
- Description: Lists linked students and summary eligibility.
...
- Refresh: Linked Student Summary
...
- Header: Clear summary-first app bar with title, linked-student switcher when multiple children are linked, and notification entry point. Screen title is 'Linked Student Summary'.
...
- Cards: Use cards to present the main units of information on 'Linked Student Summary'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Linked Student Summary' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Linked Student Summary' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/Parent_Linked_Student_Summary.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Parent_Linked_Student_Summary.mmd)
