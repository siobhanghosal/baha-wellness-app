...
- Screen ID: `B-01`
- Screen Name: `Support Queue`
- Description: Priority-sorted list of acute safety, monitoring, pastoral, and help cases.
...
- Header: Dense operational header with role context, queue counters, filters, and export or action shortcuts. Screen title is 'Support Queue'.
...
- Cards: Use cards to present the main units of information on 'Support Queue'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Support Queue' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Support Queue' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/BAHA_Support_Queue.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/BAHA_Support_Queue.mmd)
