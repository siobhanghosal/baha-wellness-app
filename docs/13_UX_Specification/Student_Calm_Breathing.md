...
- Screen ID: `S-23`
- Screen Name: `Calm Breathing`
- Description: Short guided regulation activities.
...
- Header: Top app bar with age-appropriate title, back affordance when not top-level, and optional help action. Screen title is 'Calm Breathing'.
...
- Cards: Use cards to present the main units of information on 'Calm Breathing'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Calm Breathing' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Calm Breathing' is missing or delayed.
- User loses connectivity during interaction and resumes later.
- Role entitlement changes while the session is active.

## Mermaid Reference

- Diagram File: [Mermaid/Student_Calm_Breathing.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Student_Calm_Breathing.mmd)
