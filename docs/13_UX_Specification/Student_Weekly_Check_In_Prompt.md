...
- Screen ID: `S-11`
- Screen Name: `Weekly Check-In Prompt`
- Description: Schedules and launches the weekly flow.
...
- Header: Top app bar with age-appropriate title, back affordance when not top-level, and optional help action. Screen title is 'Weekly Check-In Prompt'.
...
- Cards: Use cards to present the main units of information on 'Weekly Check-In Prompt'. Cards should expose status, summary metadata, a primary action, and a clear affordance for deeper detail.
...
- Empty States: If 'Weekly Check-In Prompt' has no data, show a role-appropriate explanation, why the state is empty, and the single next best action. Never show a blank surface.
...
- Edge Cases:
- Upstream payload for 'Weekly Check-In Prompt' is missing or delayed.
...
- Diagram File: [Mermaid/Student_Weekly_Check_In_Prompt.mmd](/Users/solomonkaruppiah/Desktop/Baha_Data/docs/13_UX_Specification/Mermaid/Student_Weekly_Check_In_Prompt.mmd)
