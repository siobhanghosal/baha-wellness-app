...
async def bootstrap_raw_storage(args: argparse.Namespace) -> None:
    from baha_rag.acquisition.raw_bootstrap import RawStorageBootstrapService
    from baha_rag.config import get_settings
    from baha_rag.db.session import SessionLocal

    async with SessionLocal() as session:
        report = await RawStorageBootstrapService(
            session,
...
async def chat_with_rag(args: argparse.Namespace) -> None:
    from baha_rag.config import get_settings
    from baha_rag.db.repository import KnowledgeRepository
    from baha_rag.db.session import SessionLocal
    from baha_rag.embeddings.bge import EmbeddingService
    ...
    from baha_rag.retrieval.hybrid import HybridRetriever
    from baha_rag.schemas import ChatMessage
    from baha_rag.taxonomy import find_conditions

    settings = get_settings()
    history: list[ChatMessage] = []
    filters = {"condition": args.condition} if args.condition else {}
    ...
    async with SessionLocal() as session:
        ...
        if args.message:
            prompts = [args.message]
        else:
            prompts = None
            print("Interactive BAHA RAG chat. Type 'exit' or 'quit' to stop.")

        while True:
            message = prompts.pop(0) if prompts else input("you> ").strip()
            if not message:
                continue
            if message.lower() in {"exit", "quit"}:
                break
            results = await retriever.search(message, top_k=args.top_k, filters=filters)
            condition = args.condition or next(iter(find_conditions(message)), "General Wellbeing")
            answer = await chat_service.generate(
                query=message,
                perspective=args.audience,
                condition=condition,
                evidence=results,
                history=history,
            )
                    ...
                    json.dumps(
                        {
                            ...
                            "retrieved": [item.model_dump(mode="json") for item in results],
                        },
                        indent=2,
                    )
                )
...
def main() -> None:
    ...
    bootstrap = subcommands.add_parser("bootstrap-raw-storage")
    bootstrap.add_argument("--root", default="storage/raw")
    bootstrap.add_argument("--limit", type=int)
    bootstrap.add_argument("--organization", action="append")
    bootstrap.add_argument(
        "--output",
        default="storage/reports/raw-storage-bootstrap-report.json",
    )
    ...
    chat.add_argument(
        "--audience",
        choices=["parent", "teacher", "counselor", "adolescent"],
        default="adolescent",
    )
    chat.add_argument("--top-k", type=int, default=8)
    chat.add_argument("--condition")
    ...
    if args.command == "ingest-url":
    ...
    elif args.command == "bootstrap-raw-storage":
        asyncio.run(bootstrap_raw_storage(args))
        ...
        asyncio.run(chat_with_rag(args))
