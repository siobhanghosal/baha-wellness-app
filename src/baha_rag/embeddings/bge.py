...
class EmbeddingService:
    ...
    def _load_model(self):
        if self._model is None:
            ...
            position_limit = int(
                getattr(self._model[0].auto_model.config, "max_position_embeddings", 512)
            )
            self._model_max_tokens = max(
                32,
                min(int(self._model.max_seq_length) - 16, position_limit - 16, 496),
            )
        return self._model
    ...
    def embed_texts(self, texts: Iterable[str]) -> list[list[float]]:
        ...
        items = self._truncate_for_encoder(items)
    ...
    def _truncate_for_encoder(self, texts: list[str]) -> list[str]:
        tokenizer = self._load_model().tokenizer
        truncated: list[str] = []
        for text in texts:
            token_ids = tokenizer.encode(text, add_special_tokens=False)
            if len(token_ids) <= self._model_max_tokens:
                truncated.append(text)
                continue
            truncated.append(
                tokenizer.decode(
                    token_ids[: self._model_max_tokens],
